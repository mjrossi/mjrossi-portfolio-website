...
- `src/layouts/Desk.astro` — the Desk's chrome and **all** of its CSS. In this file rather than `global.css` for the reason `GalleyMargin.astro` keeps its own block: `global.css` ships on every public page, and Astro decides what a stylesheet contains from the module graph, so rules put there would be weight on every reader for markup none of them can see. Only the `/admin` routes import this, so its styles end up inlined in the server bundle and there is no client stylesheet to leak. `is:global` rather than scoped — unlike the galley's `is:inline`, which was about elements created at runtime — because the markup is rendered by the *page*, in the page's own scope, and scoped rules written here would not match it. Smoke asserts no `desk-` identifier reaches a public page.
...
- `src/lib/desk.js` — `deskIndex(...)`, what earns a row on the Desk index and in what order. A **union**, not a filter over the collection: a round can outlive its draft and a link can outlive its post entirely. Unit-tested in `src/lib/desk.test.js`.
...
- `scripts/smoke/*.mjs` — the assertions, split by what they can be run against:
  ...
  - `live-desk.mjs` — "can the draft inventory leak": `/admin` closed to no token, an expired one, a malformed one, and a valid token for a **different Access application on the same team**; then open to a real one, with `no-store` + `noindex`, the fixture roster, and the notes. The negative cases fail independently on purpose.
...
**That override replaces the trust root**, which is why it is `.dev.vars` only and why `just smoke` refuses to start when `.dev.vars` sets it: wrangler prefers `.dev.vars` over `--var`, so a value there would shadow smoke's own keypair and every Desk assertion would fail locally while CI passed. Never `wrangler secret put` it. Nothing in CI can prove it is unset in production, so if the gate ever behaves oddly, check this first.
