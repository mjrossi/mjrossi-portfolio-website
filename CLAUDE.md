...
- `src/components/BlogPostEntry.astro` — shared `<article class="post-entry">` card used by `blog/index.astro` and `blog/tag/[tag].astro`. Carries **no chip row**: five or six saturated chips per entry were the first thing the eye landed on, ahead of the titles, so one primary topic moved into the meta line (`PostTopic.astro`) and the full set lives on the post itself.
...
- `src/components/PostTopic.astro` — the primary topic, inline in a post's or an entry's meta line. "Primary" is simply the **first tag in frontmatter**; the ordering there is editorial, most specific first, and it is the only signal available.
- `src/components/PostNav.astro` — previous/next by publication date, below the tag footer and the subscribe card. No related-post logic. When there is no neighbour the cell is still rendered ("Newest post" / "Oldest post") rather than collapsing the grid — a one-column nav on a two-column layout reads as a broken cell.
...
- `src/env.d.ts` — types for Cloudflare runtime `Env` (`BUTTONDOWN_API_KEY`, `TURNSTILE_SECRET_KEY`, `PREVIEW_SIGNING_KEY`, `DB`, and the Access trio `ACCESS_TEAM_DOMAIN` / `ACCESS_AUD` / `ACCESS_JWKS_OVERRIDE`) and Astro `ImportMetaEnv` (`PUBLIC_TURNSTILE_SITE_KEY`). Also augments `App.Locals` with the preview signals and `admin`; **`undefined` must always mean denied** for every one of them.
...
- `astro.config.mjs` — Cloudflare adapter, MDX integration (for the blog), sitemap integration (**filtered through `isAdminPath`**, so the Desk's per-post URLs — one per scheduled draft — never reach a public file), Astro `Font` integration for Inter / Fraunces / Source Serif 4.
...
**Order the tags most-specific-first.** The first tag is the post's *primary topic* and it is the one that renders in the meta line, on the post and on the index entry (`PostTopic.astro`). The rest appear only in the post's footer under "Filed under". Nothing validates the ordering, so getting it wrong is silent and looks like an editorial judgement.
