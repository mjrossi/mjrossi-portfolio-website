...
- **Build command** (branch-conditional; `WORKERS_CI_BRANCH` is injected by Workers Builds): `mise install && mise exec -- sh -c 'if [ "$WORKERS_CI_BRANCH" = "main" ]; then npm run build; else CLOUDFLARE_ENV=preview npm run build; fi'`. On a branch this bakes `name: mjrossi-portfolio-website-preview` (+ `workers_dev:false`, `preview_urls:true`) into the generated config.
