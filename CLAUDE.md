...
Two things about that hostname that the workflow learned the hard way, both documented in `docs/ARCHITECTURE.md`:
...
- **These hosts are behind Cloudflare Access.** Anything automated that fetches a preview URL needs the `CF_ACCESS_CLIENT_ID` / `CF_ACCESS_CLIENT_SECRET` service token, or it silently gets a login page instead of the site. Note that this Access layer, not `isPreviewHost`, is currently what keeps scheduled drafts on preview hosts away from the public.
