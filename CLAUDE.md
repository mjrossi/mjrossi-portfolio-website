...
- `public/scripts/newsletter.js` — the client half of one of the two JS carve-outs. Served as a static asset (not bundled by Astro) so it loads as an external module from `/scripts/newsletter.js` and works under the strict `script-src 'self'` CSP. Imported only by `src/components/NewsletterSignup.astro`.
...
- `src/components/GalleyMargin.astro` — review chrome (bar, notes panel, composer). Owns its own styles, like `NewsletterSignup.astro` — but in an **`is:inline`** block, which is specifically *not* scoped and is load-bearing in both directions. See "The CSS does not inherit that gate for free" under The galley.
