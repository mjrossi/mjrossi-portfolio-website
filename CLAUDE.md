...
- **`wrangler dev` died mid-run, and smoke exited 75.** Not your change. `wrangler dev`'s ProxyWorker loses its connection to a Durable Object that is called repeatedly, wrangler's ProxyController treats that as fatal, and the dev server exits partway through the run — [cloudflare/workers-sdk#4562](https://github.com/cloudflare/workers-sdk/issues/4562), open, with no fix through wrangler 4.119. Local D1 **is** such a Durable Object (miniflare keeps it under `.wrangler/state/v3/d1/miniflare-D1DatabaseObject/`), so the galley section is what trips it — roughly one run in two on CI's slower cores, and rarely on a fast local machine. There is no mock to reach for: the local database is already miniflare's SQLite fake, and the only way to leave the Durable Object path is to drop the D1 binding from the worker under test, which takes the quota and allowlist assertions with it. So smoke **exits 75 rather than 1** when the runtime dies, and `build.yml` retries that one exit code once. A failed assertion still exits 1 and is never retried. Don't broaden the retry to `|| npm run smoke` — that would turn a reproducible bug into a flaky one.

  Recognise it by the report, which now names it: `smoke: wrangler dev EXITED MID-RUN` (or `is NOT ANSWERING`), followed by the tail of wrangler's own log. Everything listed above that line is collateral — requests in flight when the runtime went away come back `500`, and the rest are refused. Before smoke knew to say so, this surfaced as four galley assertions failing, including "a valid token wrote to a post it was not minted for", which is alarming and was never true.
