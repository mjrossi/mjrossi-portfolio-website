...
**1. PR preview deploys — automatic.** Any request to a `*.workers.dev` **branch/version** hostname reveals *every* scheduled post, across the index, tag pages, post URLs, and RSS. Push a branch, open the Cloudflare preview URL, and your drafts are there. No secret and no flag — but see the dashboard caveat below, which is load-bearing. (Branch builds run on the isolated `mjrossi-portfolio-website-preview` Worker — see "Preview deploys" — so the host carries a `-preview` suffix; `isPreviewHost` still matches it because it isn't the production alias.)
...
|---|---|---|
| `PUBLIC_TURNSTILE_SITE_KEY` | Astro build (`import.meta.env`) — baked into HTML | **Local + GitHub CI:** `mise.toml` `[env]` (commits the real production site key — it's public by design). `mise.development.toml` overrides with the always-passes test key when `MISE_ENV=development` (recommended local shell setting). `mise.ci.toml` does the same when `MISE_ENV=ci` (set in `build.yml`). `mise.local.toml` (gitignored) can override anything for machine-specific testing. **Cloudflare production/preview build:** a dashboard **build variable** `PUBLIC_TURNSTILE_SITE_KEY` (the Cloudflare build does not use mise — see below). Keep the two in sync. |
...
Cloudflare Workers Builds does **not** use mise for the production/preview build. Node and the Turnstile key come from dashboard **build variables** (`NODE_VERSION=22` and `PUBLIC_TURNSTILE_SITE_KEY=<site key>`, under Settings → Build → Variables and secrets), so the Build command is a plain `npm run build` — the branch-conditional `CLOUDFLARE_ENV=preview` form (see "Preview deploys" and `wrangler.jsonc`) is the only wrapper it needs. mise governs the *local* dev shell and GitHub Actions CI (via `jdx/mise-action`), not the Cloudflare build. Because the Turnstile site key lives in two places (`mise.toml` `[env]` for local/CI, the Cloudflare build variable for production), keep them the same value.
...
**mise is the single source of truth for shell-level vars.** `mise.toml` pins Node 22 and commits the production `PUBLIC_TURNSTILE_SITE_KEY` (public by design). Two committed overrides switch in the always-passes Turnstile test key under specific contexts: `mise.development.toml` (when `MISE_ENV=development`, the recommended local-dev default) and `mise.ci.toml` (when `MISE_ENV=ci`, set in `.github/workflows/build.yml`). `mise.local.toml` (gitignored) remains available for machine-specific overrides on top of those. `jdx/mise-action` exports `[env]` to `GITHUB_ENV` so subsequent steps see the values. Cloudflare Workers Builds, by contrast, does **not** use mise — it gets Node and `PUBLIC_TURNSTILE_SITE_KEY` from dashboard build variables (`NODE_VERSION`, `PUBLIC_TURNSTILE_SITE_KEY`), so its Build command is a plain `npm run build` (branch-conditional for preview isolation).
...
PR branches get preview URLs from Cloudflare Workers Builds. Branch builds deploy to an **isolated preview Worker** — `mjrossi-portfolio-website-preview`, the `env.preview` block in `wrangler.jsonc` — never to the production Worker. This is the deploy-target firewall added after a mid-feature dashboard secret edit promoted branch code to `mjrossi.com`: a dashboard secret edit redeploys the Worker's *latest uploaded version*, so once branch versions no longer land on the production Worker, that edit can only ever re-promote `main`.

**How the env is selected — this is the non-obvious part.** Under `@astrojs/cloudflare` v14 (Astro 6) the adapter regenerates `dist/server/wrangler.json` on every `astro build` and **strips the `env` block**, so `wrangler deploy --env preview` at *deploy* time does nothing. The Cloudflare Vite plugin picks the environment at **build** time from the `CLOUDFLARE_ENV` variable instead. So isolation is driven by the **build command**, not the deploy command:

- **Build command** (branch-conditional; `WORKERS_CI_BRANCH` is injected by Workers Builds): `sh -c 'if [ "$WORKERS_CI_BRANCH" = "main" ]; then npm run build; else CLOUDFLARE_ENV=preview npm run build; fi'`. On a branch this bakes `name: mjrossi-portfolio-website-preview` (+ `workers_dev:false`, `preview_urls:true`) into the generated config. (The live Cloudflare build supplies Node and the Turnstile key via the `NODE_VERSION` and `PUBLIC_TURNSTILE_SITE_KEY` **build variables** in the dashboard — not mise — so the build command needs no mise activation. See "Newsletter → env vars" for the full local-vs-Cloudflare split.)
- **Production deploy command**: `npx wrangler deploy` (unchanged).
- **Non-production deploy command**: a **plain `npx wrangler versions upload`** — no `--env`, since the build already chose the env. This preserves the per-branch preview URL `<alias>-mjrossi-portfolio-website-preview.link00seven.workers.dev` that `lighthouse.yml` audits and `isPreviewHost` unlocks.

The preview Worker's secrets are set with `wrangler secret put --env preview` (that path *does* honor the `env.preview` block — only build/deploy routes through the adapter-generated config) and **mirror production**, so a subscription via a preview URL still lands in the production Buttondown account — acceptable for a personal site (preview URLs are `noindex`'d). Its SESSION KV namespace is auto-provisioned by the Workers Builds token on the first preview build. To fully isolate the mailing list too, give the preview env a separate Buttondown key.
