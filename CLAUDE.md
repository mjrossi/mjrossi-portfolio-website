...
The split is load-bearing on operator config that lives in the Cloudflare dashboard, not the repo: the **non-production** deploy command must be `npx wrangler versions upload --env preview` (production stays `npx wrangler deploy`). `versions upload` — not `deploy` — is what preserves the per-branch preview URL `<alias>-mjrossi-portfolio-website-preview.link00seven.workers.dev` that `lighthouse.yml` audits and `isPreviewHost` unlocks. The preview Worker's secrets are set with `wrangler secret put --env preview` and **mirror production**, so a subscription via a preview URL still lands in the production Buttondown account — acceptable for a personal site (preview URLs are `noindex`'d). To fully isolate that too, give the preview env a separate Buttondown key.
