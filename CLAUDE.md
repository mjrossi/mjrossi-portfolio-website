...
- `migrations/0002_preview_link_ceiling.sql` — adds `preview_links.max_exp`, the signed ceiling that makes a link extendable. `NULL` means a row minted before ceilings existed: its token expires when its row does, so it has no headroom and `extendLink` refuses it — historically accurate and fail-closed in the same direction. **The worker does not need this column**; it enforces the ceiling from the token and the expiry from `exp`, both of which predate this migration. Minting does need it, so apply it before the next `just preview-link`.
...
**There are two clocks, and the split is what makes extending possible.**

> The token says *this link can never work past X*. The row says *and right now it works until Y ≤ X*. Extending moves Y. Past X you mint a new link.

`X` is the token's signed `exp` — the ceiling, set 30 days out at mint (`CEILING_HOURS`), immutable because it is inside the HMAC. `Y` is `preview_links.exp`, which `isLinkActive` checks on every request and `just preview-extend` rewrites with a single `UPDATE`. **The URL never changes**, so there is nothing to re-send and the reviewer never learns their link was about to lapse. `--hours` is a new window from *now*, so the same command shortens as readily as it extends. The ceiling is stored as `max_exp` purely so the CLI can refuse an extension the worker would silently reject on arrival — that refusal looks identical to the feature being broken.
