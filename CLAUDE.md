...
- `scripts/links-db.mjs` — the only owner of `preview_links` SQL (`recordLinks`, `revokeLinks`, `extendLink`, `getLink`, `listLinks`, `listAllLinks`, `clearLinks`). Validates its own inputs (`SLUG_RE`, `LINK_ID_RE`, `Number.isInteger`), because wrangler's `--command` takes a string rather than bound parameters and that shape check is what makes the interpolation safe. `smoke.mjs` seeds fixtures through the same `recordLinks` production mints through, and runs `extendLink` end to end there too — it is the only place this module's SQL executes under test, since the module shells out to wrangler at import time and `node --test` can't load it. **`extendLink` enforces the ceiling inside its own `UPDATE`** (`<exp> <= max_exp`), not in the calling script: a read-then-write would be two round-trips with a window between them, and would move the one invariant stopping an extendable link from becoming a permanent one out of the module that owns the table. Same argument as the galley write quota being a single `INSERT ... SELECT`. `getLink` exists only to *explain* a refusal, never to gate one.
...
- `scripts/notes-db.mjs` — the only owner of `galley_notes` SQL (`closeNotes`, `reopenNote`, `listNotes`, `seedNotes`, `clearNotes`, `noteIdsInFile`). Same shape and same argument as `links-db.mjs` one table over: wrangler's `--command` takes a string rather than bound parameters, so this file shape-checks every value that reaches a statement. `noteIdsInFile` is the file read; the scan it wraps is `src/lib/galley-manifest.js`. Smoke seeds and closes through these same functions — not because the module can't be imported under `node --test` (it can) but because every function in it shells out to `wrangler d1 execute` when *called*, so running the SQL needs a migrated local database.
