...
- `lighthouse.yml` — fires on `check_suite` completion when the Cloudflare Workers Builds check finishes (and on `workflow_dispatch` for ad-hoc audits of any URL). Audits six routes (`/`, `/work`, `/education`, `/urban-mobility`, `/blog`, and one post — `/blog/why-im-pivoting`). `/projects` is excluded because it's `noindex` and would fail SEO by design. The blog pair is what covers the newsletter form's Turnstile script and the `BlogPost` layout; keep the list here in sync with the `urls:` block in `.github/workflows/lighthouse.yml`.
  ...
  - **`numberOfRuns` is 3 in both configs, and this is not tuning.** At one run a noisy shared runner is enough to fail the build on its own: with identical content, `/` has scored 97, 97, 65, 72, 97 across consecutive runs, the low samples showing >1s of TBT on a page that ships no client JS. Three samples make a single bad one harmless. Both configs also pin **`aggregationMethod: "median-run"`**, which is *not* the LHCI default — the default is `optimistic` (best of N). Left at the default the gate would assert against the best run while the PR comment reported the median, so the two disagreed by several points in the direction that looks worse (a real run gated on 0.87 while the comment showed 84). The summary step collapses each URL to that same median-by-performance run, so the table and the check now describe one run rather than two.
  ...
  - **Credential containment.** Lighthouse copies `settings.extraHeaders` verbatim into `configSettings` in every report JSON, and the HTML reports embed that same JSON. Note that `lhci` writes reports under **two** naming schemes and both carry the headers: `<url-slug>--<timestamp>.report.json` from collect, and `lhr-<ms>.json` dumped by upload — a redaction glob matching only one of them silently redacts nothing. This repo is public, so on runs where the token is in play the action's own upload paths are both disabled (`temporaryPublicStorage` writes to a world-readable GCS URL printed in the logs; artifacts on a public repo are downloadable by anyone). A redaction step nulls the field across every `.json`, drops the HTML, and then **greps the directory and fails the job** — once for the header name and once for the secret's literal value — if anything survived. It is the only thing between a service token and a public artifact, so it verifies rather than assumes. `main` injects no headers and keeps public reports and HTML exactly as before.
  ...
  - **Credentials and config are chosen by target host, not by branch.** A `*.workers.dev` host gets the token and the relaxed config; `mjrossi.com` gets neither. Keying off the ref instead would break the `workflow_dispatch` escape hatch, which is normally fired from `main` against a preview URL.
