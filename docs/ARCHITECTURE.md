...
Reference for how mjrossi.com is built, deployed, and quality-checked. Read alongside `CLAUDE.md` (conventions + design system), [FILE-MAP.md](FILE-MAP.md) (every file and the decision it carries), [PREVIEW-LINKS.md](PREVIEW-LINKS.md), [GALLEY.md](GALLEY.md) (editorial review), [NEWSLETTER.md](NEWSLETTER.md), [ENVIRONMENT.md](ENVIRONMENT.md), and `README.md` (commands).
...
- `public/og.png` — 1200×630 social card. Regenerate with `node scripts/make-og.mjs public/og.png` (uses `sharp` + an inline SVG that mirrors the Broadsheet masthead, then composites the avatar and tiled noise overlay). The card intentionally carries no edition line and no location — identity only — so it doesn't drift between regenerations; scrapers cache by image URL and never re-fetch, so a stale fact on it cannot be recalled. `src/layouts/Base.astro` references it as `/og.png?v=N`; bump N when the card changes.
...
- `scripts/make-og.mjs` — regenerates `public/og.png` (1200×630). Mirrors the live masthead at OG dimensions: amber band with tiled noise overlay, Fraunces-style serif name with accented surname, italic tagline framed by rules, editorial pull quote, Broadsheet footer. The tagline and colophon are imported from `src/lib/identity.js`, the single spelling shared with `Base.astro` — the two are rendered by different engines (HTML vs SVG), and a sentence spelled once per engine drifts. Falls back to Georgia/system-ui because the site fonts aren't installed in Node — close enough at OG sizes. Intentionally omits the edition line *and* any location so the card stays timeless — it states identity only, never a fact about the present. Guarded by a source grep in `scripts/smoke/static.mjs`, since nothing can read text out of a PNG.
