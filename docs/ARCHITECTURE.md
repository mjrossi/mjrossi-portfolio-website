...
- `lighthouse.yml` — fires on `check_suite` completion when the Cloudflare Workers Builds check finishes (and on `workflow_dispatch` for ad-hoc audits of any URL). Audits six routes (`/`, `/work`, `/education`, `/urban-mobility`, `/blog`, and one post — `/blog/why-im-pivoting`). `/projects` is excluded because it's `noindex` and would fail SEO by design. The blog pair is what covers the newsletter form's Turnstile script and the `BlogPost` layout; keep the list here in sync with the `urls:` block in `.github/workflows/lighthouse.yml`.
  ...
  - **`numberOfRuns` is 3 in both configs, and this is not tuning.** At one run a noisy shared runner is enough to fail the build on its own: with identical content, `/` has scored 97, 97, 65, 72, 97 across consecutive runs, the low samples showing >1s of TBT on a page that ships no client JS. Three samples make a single bad one harmless. Both configs also pin **`aggregationMethod: "median"`**, which is *not* the LHCI default — the default is `optimistic` (best of N). Left at the default the gate asserts against the best run while the PR comment reports the median, so the two disagree by several points in the direction that looks worse (a real run gated on 0.87 while the comment showed 84).

    Use `"median"` and not `"median-run"`, which looks like the more precise setting and is a **no-op for these configs**. `median-run` swaps in the median LHRs only on the audit-assertion path; a `categories:*` assertion — which is all either config asserts — goes through `getCategoryAssertionResults()`, is handed the full LHR set, and falls through to `Math.max(...)`, i.e. exactly the `optimistic` default. Measured against `@lhci/utils` 0.15.1 with runs scoring .87/.84/.72, `median-run` and `optimistic` both assert **0.87** while `median` asserts **0.84**. The summary step reimplements LHCI's median (sort ascending; odd N takes the middle, even N averages the two middle) per category, so the table and the gate now report the same number. A row is therefore four independent per-category medians, not one run's scorecard — the same way the gate aggregates.
  - **Preview deploys sit behind Cloudflare Access, and the audit needs a service token to get past it.** Without one, every request 302s to `<team>.cloudflareaccess.com` and Lighthouse audits the *login page* — a flat A11y 77 / BP 93 / SEO 54 on all six URLs, which can never satisfy the `minScore: 1` assertions. The `CF_ACCESS_CLIENT_ID` / `CF_ACCESS_CLIENT_SECRET` repo secrets are injected as request headers into a generated `lighthouserc.runtime.json`; the Access application needs a **Service Auth** policy admitting that token (an Allow policy will not work). If the secrets are unset the workflow warns and the preview audit fails at the preflight step rather than silently scoring the wrong page.
  - **Credential containment.** Lighthouse copies `settings.extraHeaders` verbatim into `configSettings` in every report JSON, and the HTML reports embed that same JSON. Note that `lhci` writes reports under **two** naming schemes and both carry the headers: `lhr-<ms>.json` written by `lhci collect`, and `<host>-<path>-<datetime>.report.json` written by `lhci upload --target=filesystem` — a redaction glob matching only one of them silently redacts nothing. Both are always present, because `treosh/lighthouse-ci-action` runs that filesystem upload *unconditionally*, outside the block guarded by the upload toggles; do not narrow the glob on the theory that turning uploads off removes one set. This repo is public, so on runs where the token is in play the action's own upload paths are both disabled (`temporaryPublicStorage` writes to a world-readable GCS URL printed in the logs; artifacts on a public repo are downloadable by anyone). A redaction step nulls the field across every `.json`, drops the HTML, and then **greps the directory and fails the job** — once for the header name and once for each credential's literal value — if anything survived. It is the only thing between a service token and a public artifact, so it verifies rather than assumes. `main` injects no headers and keeps public reports and HTML exactly as before.

    One consequence worth knowing when reading artifacts: the action's own `uploadArtifacts` runs *before* the filesystem dump, so `main`'s `lighthouse-results` artifact holds only the collect-time `lhr-*.{json,html}`. The workflow's own upload on preview runs afterwards and holds `*.report.json` + `manifest.json` + `assertion-results.json` with all HTML stripped. Same artifact name, different contents.
  - **Fork PRs are refused, not audited.** The audit target is a deployed Worker and it is handed the Access token as a request header; a Worker built from a fork is attacker-controlled and could read that header straight off the request. The context step fails with a named error when the head PR's owner isn't this repo's owner.
  - **Credentials and config are chosen by target host, not by branch.** A `*.link00seven.workers.dev` host gets the token and the relaxed config; `mjrossi.com` gets neither. Keying off the ref instead would break the `workflow_dispatch` escape hatch, which is normally fired from `main` against a preview URL. The credential match is anchored on the **account subdomain**, not on `.workers.dev`: that namespace is shared and `workflow_dispatch` takes a free-text URL, so a bare `*.workers.dev` would let anyone with repo write access point the preflight at a Worker of their own and be handed the service token.
  - A preflight step curls the target before the audit and fails with a named cause if it returns non-200 or redirects off-origin. Both historical failure modes — an Access login page and an unresolvable hostname — were silent; this makes them one-line errors.
