...
- `lighthouse.yml` — fires on `check_suite` completion when the Cloudflare Workers Builds check finishes (and on `workflow_dispatch` for ad-hoc audits of any URL). Audits six routes (`/`, `/work`, `/education`, `/urban-mobility`, `/blog`, and one post — `/blog/why-im-pivoting`). `/projects` is excluded because it's `noindex` and would fail SEO by design. The blog pair is what covers the newsletter form's Turnstile script and the `BlogPost` layout; keep the list here in sync with the `urls:` block in `.github/workflows/lighthouse.yml`.
  ...
  - **`numberOfRuns` is 3 in both configs, and this is not tuning.** At one run a noisy shared runner is enough to fail the build on its own: with identical content, `/` has scored 97, 97, 65, 72, 97 across consecutive runs, the low samples showing >1s of TBT on a page that ships no client JS. LHCI asserts against the median run, so three samples make a single bad one harmless. The summary step collapses each URL to its median-by-performance run for the same reason — otherwise the comment table would list every URL three times.
  ...
  - **Credential containment.** Lighthouse copies `settings.extraHeaders` verbatim into `configSettings` in every `lhr-*.json`, and the HTML reports embed that same JSON. This repo is public, so on runs where the token is in play the action's own upload paths are both disabled (`temporaryPublicStorage` writes to a world-readable GCS URL printed in the logs; artifacts on a public repo are downloadable by anyone). A redaction step nulls the field, drops the HTML, and then **greps the directory and fails the job** if the header name survived — it is the only thing between a service token and a public artifact, so it verifies rather than assumes. `main` injects no headers and keeps public reports and HTML exactly as before.
